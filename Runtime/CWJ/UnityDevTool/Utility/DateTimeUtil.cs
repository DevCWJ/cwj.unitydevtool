...
namespace CWJ
{
	public static class DateTimeUtil
	{
		...
		internal static string GetDateTimeFormatString(DateTime dateTime, double range)
		{
			...
			if (range >= DateTimeUtil.ONE_YEAR * DateTimeUtil.MIN_TIME_SPLIT_NUMBER)
			{
				...
				if (range >= DateTimeUtil.ONE_MONTH * DateTimeUtil.MIN_TIME_SPLIT_NUMBER)
				{
					...
					if (range >= DateTimeUtil.ONE_DAY * DateTimeUtil.MIN_TIME_SPLIT_NUMBER)
					{
						...
						if (range >= DateTimeUtil.ONE_HOUR * DateTimeUtil.MIN_TIME_SPLIT_NUMBER)
						{
							...
							if (range >= DateTimeUtil.ONE_MINUTE * DateTimeUtil.MIN_TIME_SPLIT_NUMBER)
							{
								dateString = dateTime.ToString(s_SecondDateFormatter);
							}
							else
		...
		/// <summary>
		/// Calculate the appropriate Tick value based on the given maximum and minimum timestamp range
		/// </summary>
		...
		internal static float UpdateTimeAxisDateTimeList(List<double> list, int minTimestamp, int maxTimestamp, int splitNumber)
		{
			...
			if (range >= ONE_YEAR * MIN_TIME_SPLIT_NUMBER)
			{
				...
				if (range >= ONE_MONTH * MIN_TIME_SPLIT_NUMBER)
				{
					...
					if (range >= ONE_DAY * MIN_TIME_SPLIT_NUMBER)
					{
						...
						if (range >= ONE_HOUR * MIN_TIME_SPLIT_NUMBER)
						{
							...
							if (range >= ONE_MINUTE * MIN_TIME_SPLIT_NUMBER)
							{
