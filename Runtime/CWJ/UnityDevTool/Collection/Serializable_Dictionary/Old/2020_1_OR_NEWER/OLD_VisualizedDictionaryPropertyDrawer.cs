...
namespace CWJ.Collection
{
	...
	/// for unity 2020
	/// </summary>
	[CustomPropertyDrawer(typeof(OLD_DictionaryVisualized<,>), true)]
	public class OLD_VisualizedDictionaryPropertyDrawer : PropertyDrawer_CWJ
	{
				...
				if (conflictOriginLength > 0 || nullKeyLength > 0)
				{
					drawConflictInfo = (itemIndex, lineRect) =>
					{
						...
						if (conflictOriginIndexes.IsExists(itemIndex))
						{
							...
							if (conflictWarningIndexes.IsExists(itemIndex))
							{
								icon = s_warningIcon_Conflict;
							}
							else
								if (nullKeyIndexes.IsExists(itemIndex))
								{
